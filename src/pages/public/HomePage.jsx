...
export const HomePage = () => {
  ...
  const slides = [
    {image: "/hero_aurora_cabin.jpg",headline: <>Looking for <br /><span className="text-[#a0d2b4] italic font-serif">Visa Experts?</span></>,subtext: "From document verification to application submission, our experts make your visa process faster, simpler, and stress-free.",theme: "dark",},
    // {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const { trackConversion } = await import("../../utils/tracking");
                    ...
                    return (
                      <div key={index} className={`col-start-1 row-start-1 flex flex-col transition-all duration-700 delay-300 transform ${isActive ? "translate-y-0 opacity-100 z-10" : "translate-y-8 opacity-0 z-0 pointer-events-none"} ${isDark ? "text-white" : "text-gray-900"}`}>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                          <a href="https://wa.me/971557338429" target="_blank" rel="noopener noreferrer" onClick={() => { import("../../utils/tracking").then(m => m.trackWhatsAppClick()); }} className="inline-flex items-center justify-center gap-2 px-6 py-3 text-sm bg-[#1a4a38] hover:bg-[#133829] text-white rounded-full font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5">
