        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="bg-[#0B1424] border border-[#1A2B47] rounded-[16px] h-72 animate-pulse flex flex-col p-6 space-y-6"
              >
                <div className="h-6 bg-[#1A2B47] rounded w-2/3"></div>
                <div className="h-4 bg-[#1A2B47] rounded w-5/6"></div>
                <div className="h-0.5 bg-[#1A2B47] w-10"></div>
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="space-y-2">
                    <div className="h-5 bg-[#1A2B47] rounded w-1/2"></div>
                    <div className="h-3.5 bg-[#1A2B47] rounded w-3/4"></div>
                  </div>
                  <div className="space-y-2">
                    <div className="h-5 bg-[#1A2B47] rounded w-1/2"></div>
                    <div className="h-3.5 bg-[#1A2B47] rounded w-3/4"></div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
            ...
            <p className="text-sm text-[#EDE0C4]/60">Unable to load visa services. Please refresh the page.</p>
            <button
              onClick={() => setRetryTrigger((prev) => prev + 1)}
              className="px-6 py-2.5 bg-gradient-to-r from-[#C9A84C] to-[#E2BC6A] text-[#070D1A] font-bold rounded-lg hover:opacity-95 transition-all text-xs uppercase tracking-wider"
            >
              Retry Loading
            </button>
          </div>
          ...
          /* Empty State */
          <div className="glass-card bg-[#0B1424] border border-[#1A2B47] rounded-[16px] p-12 max-w-lg mx-auto text-center space-y-6">
            <div className="mx-auto h-16 w-16 bg-[#1A2B47] text-[#C9A84C] rounded-full flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {visaTypes.map((visa) => {
              ...
              return (
                <div
                  key={visa.id}
                  className="bg-[#0B1424] border border-[#1A2B47] rounded-[16px] overflow-hidden hover:border-[#C9A84C] hover:shadow-[0_0_20px_rgba(201,168,76,0.15)] transition-all duration-300 flex flex-col justify-between group animate-[fadeIn_0.3s_ease-out]"
                >
                  {/* Card Cover Image */}
                  <div className="h-48 w-full overflow-hidden bg-[#1A2B47]/20 relative flex-shrink-0">
                    {visa.imageUrl ? (
                      <img
                        src={visa.imageUrl}
                        alt={visa.name}
                      ...
                      <div className="w-full h-full flex items-center justify-center text-[#EDE0C4]/30 uppercase tracking-widest text-[9px] font-bold">
                        Eshaare Tours & Visas
                      </div>
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0B1424] via-transparent to-transparent"></div>
                  </div>

                  {/* Card Text Content */}
                  <div className="p-6 pt-4 flex-1 flex flex-col justify-between space-y-4">
                    <div className="space-y-4">
                      <h3 className="text-xl font-bold font-display text-white group-hover:text-[#C9A84C] transition-colors leading-snug">
                        {visa.name}
                      </h3>
                      <p className="text-xs text-[#EDE0C4]/60 line-clamp-2 leading-relaxed">
                        {visa.tagline}
                      </p>

                      {/* Gold Divider */}
                      <div className="h-[1px] bg-[#C9A84C] w-10"></div>

                      {/* First 2 Stats badged side-by-side */}
                      <div className="grid grid-cols-2 gap-4 py-2 border-b border-[#1A2B47]/60 pb-2">
                        <div className="flex items-start gap-2">
                          <StatIcon name={firstStat.icon} className="h-4.5 w-4.5 text-[#C9A84C] mt-0.5" />
                          <div>
                            <div className="text-base font-bold text-white font-mono leading-none">{firstStat.value}</div>
                            <span className="text-[9px] text-[#EDE0C4]/45 font-bold uppercase tracking-wider">{firstStat.label}</span>
                          </div>
                        </div>
                        <div className="flex items-start gap-2">
                          <StatIcon name={secondStat.icon} className="h-4.5 w-4.5 text-[#C9A84C] mt-0.5" />
                          <div>
                            <div className="text-base font-bold text-white font-mono leading-none">{secondStat.value}</div>
                            <span className="text-[9px] text-[#EDE0C4]/45 font-bold uppercase tracking-wider">{secondStat.label}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-4">
                      <Link
                        to={`/visa/${visa.slug}`}
                        className="w-full text-center py-2.5 border border-[#1A2B47] text-white hover:border-[#C9A84C] hover:text-[#C9A84C] rounded-lg text-xs font-bold uppercase tracking-wider transition-all"
                      >
                        Learn More
                      </Link>
                      <Link
