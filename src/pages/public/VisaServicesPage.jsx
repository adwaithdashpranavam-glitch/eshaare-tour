...
export const VisaServicesPage = () => {
  ...
  return (
    <div className="bg-[#2C2712] min-h-screen text-[#EEDADA] font-sans pb-24">
      {/* Page Hero Banner */}
      <section className="relative py-28 overflow-hidden bg-[#1C190B] border-b border-[#453E1D] text-center">
        {/* Background Image */}
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-[#2C2712]/85 backdrop-blur-[2px]"></div>
        </div>
        ...
        <div className="relative z-10 max-w-container-max mx-auto px-4 space-y-4">
          ...
          <p className="text-sm md:text-base text-[#EEDADA]/80 max-w-xl mx-auto leading-relaxed">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="bg-[#1C190B] border border-[#453E1D] rounded-[16px] h-72 animate-pulse flex flex-col p-6 space-y-6"
              >
                <div className="h-6 bg-[#453E1D] rounded w-2/3"></div>
                <div className="h-4 bg-[#453E1D] rounded w-5/6"></div>
                <div className="h-0.5 bg-[#453E1D] w-10"></div>
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="space-y-2">
                    <div className="h-5 bg-[#453E1D] rounded w-1/2"></div>
                    <div className="h-3.5 bg-[#453E1D] rounded w-3/4"></div>
                  </div>
                  <div className="space-y-2">
                    <div className="h-5 bg-[#453E1D] rounded w-1/2"></div>
                    <div className="h-3.5 bg-[#453E1D] rounded w-3/4"></div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
            ...
            <p className="text-sm text-[#EEDADA]/60">Unable to load visa services. Please refresh the page.</p>
            <button
              onClick={() => setRetryTrigger((prev) => prev + 1)}
              className="px-6 py-2.5 bg-gradient-to-r from-[#EAC784] to-[#EDB868] text-[#2C2712] font-bold rounded-lg hover:opacity-95 transition-all text-xs uppercase tracking-wider"
            >
              Retry Loading
            </button>
          </div>
          ...
          /* Empty State */
          <div className="glass-card bg-[#1C190B] border border-[#453E1D] rounded-[16px] p-12 max-w-lg mx-auto text-center space-y-6">
            <div className="mx-auto h-16 w-16 bg-[#453E1D] text-[#EAC784] rounded-full flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {visaTypes.map((visa) => {
              ...
              return (
                <div
                  key={visa.id}
                  className="bg-[#1C190B] border border-[#453E1D] rounded-[16px] overflow-hidden hover:border-[#EAC784] hover:shadow-[0_0_20px_rgba(201,168,76,0.15)] transition-all duration-300 flex flex-col justify-between group animate-[fadeIn_0.3s_ease-out]"
                >
                  {/* Card Cover Image */}
                  <div className="h-48 w-full overflow-hidden bg-[#453E1D]/20 relative flex-shrink-0">
                    {visa.imageUrl ? (
                      <img
                        src={visa.imageUrl}
                        alt={visa.name}
                      ...
                      <div className="w-full h-full flex items-center justify-center text-[#EEDADA]/30 uppercase tracking-widest text-[9px] font-bold">
                        Eshaare Tours & Visas
                      </div>
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-[#1C190B] via-transparent to-transparent"></div>
                  </div>

                  {/* Card Text Content */}
                  <div className="p-6 pt-4 flex-1 flex flex-col justify-between space-y-4">
                    <div className="space-y-4">
                      <h3 className="text-xl font-bold font-display text-white group-hover:text-[#EAC784] transition-colors leading-snug">
                        {visa.name}
                      </h3>
                      <p className="text-xs text-[#EEDADA]/60 line-clamp-2 leading-relaxed">
                        {visa.tagline}
                      </p>

                      {/* Gold Divider */}
                      <div className="h-[1px] bg-[#EAC784] w-10"></div>

                      {/* First 2 Stats badged side-by-side */}
                      <div className="grid grid-cols-2 gap-4 py-2 border-b border-[#453E1D]/60 pb-2">
                        <div className="flex items-start gap-2">
                          <StatIcon name={firstStat.icon} className="h-4.5 w-4.5 text-[#EAC784] mt-0.5" />
                          <div>
                            <div className="text-base font-bold text-white font-mono leading-none">{firstStat.value}</div>
                            <span className="text-[9px] text-[#EEDADA]/45 font-bold uppercase tracking-wider">{firstStat.label}</span>
                          </div>
                        </div>
                        <div className="flex items-start gap-2">
                          <StatIcon name={secondStat.icon} className="h-4.5 w-4.5 text-[#EAC784] mt-0.5" />
                          <div>
                            <div className="text-base font-bold text-white font-mono leading-none">{secondStat.value}</div>
                            <span className="text-[9px] text-[#EEDADA]/45 font-bold uppercase tracking-wider">{secondStat.label}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-4">
                      <Link
                        to={`/visa/${visa.slug}`}
                        className="w-full text-center py-2.5 border border-[#453E1D] text-white hover:border-[#EAC784] hover:text-[#EAC784] rounded-lg text-xs font-bold uppercase tracking-wider transition-all"
                      >
                        Learn More
                      </Link>
                      <Link
                        to={`/visa-services/${visa.slug}`}
                        className="w-full text-center py-2.5 bg-gradient-to-r from-[#EAC784] to-[#EDB868] text-[#2C2712] font-bold rounded-lg text-xs uppercase tracking-wider hover:opacity-95 transition-all shadow-sm flex items-center justify-center"
                      >
                        Apply Now
                      </Link>
                    </div>
